      ...
      <SafeAreaView edges={['top']} style={{ flex: 1 }}>
        ...
        >
          ...
          <View style={styles.matchHeader}>
            ...
            />
            <LinearGradient
              colors={['rgba(6,10,19,0.3)', 'rgba(6,10,19,0.8)', '#060A13']}
              style={StyleSheet.absoluteFillObject}
            />
            ...
            <View style={styles.navBar}>
              ...
              <Pressable
                onPress={() => { Haptics.selectionAsync(); toggleFavorite(match.id); }}
                style={styles.navBtn}
              >
                <Ionicons
                  name={isFavorite(match.id) ? 'heart' : 'heart-outline'}
                  size={22}
                  color={isFavorite(match.id) ? theme.live : '#FFF'}
                />
              </Pressable>
            </View>
          ...
          <View style={styles.tabContent}>
            {activeTab === 'stream' ? (
              <StreamTab
                ...
                botStatus={botStatus}
                onRefresh={() => id ? refreshMatchStreams(id) : undefined}
              />
            ) : null}
...
// ==========================================
// STREAM TAB - Now with real scraped streams
// ==========================================
...
function StreamTab({
  ...
  return (
    <View>
      ...
      <View style={styles.playerContainer}>
        ...
        <View style={styles.playerOverlay}>
          {selectedStream ? (
            <Pressable
              ...
              onPress={() => handleOpenStream(selectedStream.url)}
            >
        ...
        {selectedStream ? (
          <View style={styles.playerControls}>
            <View style={styles.playerQuality}>
              <Text style={styles.qualityText}>{selectedStream.quality}</Text>
            </View>
      ...
      {selectedStream ? (
        <Pressable
          style={({ pressed }) => [styles.openBrowserBtn, pressed && { opacity: 0.8 }]}
          onPress={() => handleOpenStream(selectedStream.url)}
        >
          <Ionicons name="open-outline" size={18} color="#060A13" />
          <Text style={styles.openBrowserText}>{t('openInBrowser')}</Text>
        </Pressable>
      ) : null}

      {/* Scraped Stream Servers */}
      ...
      {streams.length > 0 ? (
        streams.map(stream => (
          <Pressable
            ...
            style={[
              styles.serverCard,
              selectedStreamId === stream.id && styles.serverSelected,
            ]}
          >
            <View style={[styles.serverTop, isRTL && styles.rowReverse]}>
              <View style={[styles.serverIconWrap, selectedStreamId === stream.id && styles.serverIconActive]}>
                <Ionicons
                  name="server"
                  size={16}
                  color={selectedStreamId === stream.id ? theme.primary : theme.textMuted}
                />
              </View>
              <View style={[styles.serverInfo, isRTL && { alignItems: 'flex-end' }]}>
                <Text style={[styles.serverName, selectedStreamId === stream.id && { color: theme.primary }, isRTL && styles.textRight]}>
                  {stream.name}
                </Text>
                <View style={[styles.serverMeta, isRTL && styles.rowReverse]}>
                  <View style={styles.qualityTag}>
                    <Text style={styles.qualityTagText}>{stream.quality}</Text>
                  </View>
              ...
              {selectedStreamId === stream.id ? (
                ...
                <Pressable onPress={() => handleOpenStream(stream.url)} hitSlop={8}>
        ...
        <View style={[styles.botStatsRow, isRTL && styles.rowReverse]}>
          <View style={styles.botStatItem}>
            ...
            <Text style={styles.botStatText}>
              {botStatus.lastScrapeCount} {t('matchesScanned')}
            </Text>
          </View>
          ...
          <View style={styles.botStatItem}>
            ...
            <Text style={styles.botStatText}>
              {botStatus.totalStreams} {t('streamsDetected')}
            </Text>
          </View>
        </View>
      ) : null}
    ...
    <View>
      {sortedEvents.map((event, index) => (
        <View key={index} style={[styles.eventRow, isRTL && styles.rowReverse]}>
          ...
          <View style={[
            styles.eventIcon,
            event.type === 'goal' && styles.eventIconGoal,
            event.type === 'red' && styles.eventIconRed,
            event.type === 'yellow' && styles.eventIconYellow,
          ]}>
            <Ionicons
              name={event.type === 'goal' ? 'football' : event.type === 'substitution' ? 'swap-horizontal' : 'square'}
              size={14}
              color={event.type === 'goal' ? '#060A13' : event.type === 'red' ? '#FFF' : event.type === 'yellow' ? '#060A13' : theme.textSecondary}
            />
          </View>
          <View style={[styles.eventInfo, isRTL && { alignItems: 'flex-end' }]}>
            ...
            <Text style={[styles.eventTeam, isRTL && styles.textRight]}>
...
function LineupsTab({ match }: { match: ProcessedMatch }) {
  ...
  useEffect(() => {
    fetchLineups(match.fixtureId).then(data => {
      setLineups(data);
      setLoading(false);
    });
  }, [match.fixtureId]);
  ...
  if (loading) {
    return (
      <View style={styles.emptyTab}>
        <ActivityIndicator size="small" color={theme.primary} />
        <Text style={styles.emptyTabSub}>{t('loading')}</Text>
      </View>
    );
  }
  ...
  if (lineups.length < 2) {
    return (
      <View style={styles.emptyTab}>
        ...
        <Text style={styles.emptyTabText}>{t('lineups')}</Text>
        <Text style={styles.emptyTabSub}>{t('eventsAppear')}</Text>
      </View>
    );
  }
  ...
  return (
    <View>
      ...
      {Array.from({ length: maxLen }).map((_, i) => {
        const hPlayer = homePlayers[i]?.player?.name || '';
        const aPlayer = awayPlayers[i]?.player?.name || '';
        const hNum = homePlayers[i]?.player?.number || '';
        const aNum = awayPlayers[i]?.player?.number || '';
        return (
          <View key={i} style={styles.lineupRow}>
            <Text style={styles.lineupNumber}>{hNum}</Text>
            <Text style={styles.lineupPlayer}>{hPlayer}</Text>
            <Text style={styles.lineupPlayer}>{aPlayer}</Text>
            <Text style={styles.lineupNumber}>{aNum}</Text>
          </View>
        );
      })}
    </View>
...
function StatsTab({ match }: { match: ProcessedMatch }) {
  ...
  useEffect(() => {
    fetchStatistics(match.fixtureId).then(data => {
      setStats(data);
      setLoading(false);
    });
  }, [match.fixtureId]);
  ...
  if (stats.length < 2) {
    return (
      <View style={styles.emptyTab}>
        ...
        <Text style={styles.emptyTabText}>{t('stats')}</Text>
        <Text style={styles.emptyTabSub}>{t('eventsAppear')}</Text>
      </View>
    );
  }
      ...
      {statPairs.map((stat: any, index: number) => {
        ...
        return (
          <View key={index} style={styles.statRow}>
            <Text style={[styles.statValue, homeWidth > awayWidth && styles.statValueHighlight]}>
              {stat.home}
            </Text>
            ...
            <Text style={[styles.statValue, awayWidth > homeWidth && styles.statValueHighlight]}>
              {stat.away}
            </Text>
          </View>
        );
...
const styles = StyleSheet.create({
  ...
  matchHeader: {
    height: 340, justifyContent: 'flex-end',
    alignItems: 'center', paddingBottom: 20, paddingHorizontal: 16,
  },
  navBar: {
  ...
  statusBadge: {
    ...
    borderRadius: theme.radius.full, gap: 6, marginBottom: 16,
  },
  ...
  tabBar: {
    ...
    borderBottomColor: theme.border, paddingHorizontal: 8,
  },
  tab: {
    flex: 1, flexDirection: 'row', alignItems: 'center',
    justifyContent: 'center', paddingVertical: 14, gap: 5,
  },
  ...
  botBanner: {
    ...
    backgroundColor: 'rgba(0,230,118,0.06)', borderRadius: theme.radius.lg,
  ...
  // Player
  playerContainer: {
    height: 210, borderRadius: theme.radius.xl,
  ...
  openBrowserText: { fontSize: 15, fontWeight: '700', color: '#060A13' },
  ...
  serverSelected: {
    borderColor: theme.primary, backgroundColor: 'rgba(0,230,118,0.05)',
  },
  serverTop: {
    flexDirection: 'row', alignItems: 'center', gap: 10,
  },
  ...
  qualityTag: {
    backgroundColor: 'rgba(0,230,118,0.1)',
    paddingHorizontal: 7, paddingVertical: 2, borderRadius: theme.radius.sm,
  },
  ...
  retryBtn: {
    ...
    backgroundColor: 'rgba(0,230,118,0.1)', paddingHorizontal: 16,
    paddingVertical: 8, borderRadius: theme.radius.full,
  },
  ...
  botStatsRow: {
    ...
    padding: 12, marginTop: 12, borderWidth: 1, borderColor: theme.border,
  },
  ...
  streamInfoNote: {
    ...
    marginTop: 12, backgroundColor: theme.surface, padding: 14,
    borderRadius: theme.radius.md, borderWidth: 1, borderColor: theme.border,
  },
  ...
  lineupRow: {
    ...
    borderBottomWidth: 1, borderBottomColor: 'rgba(30,42,66,0.4)',
  },
  ...
  statBarContainer: {
    flexDirection: 'row', width: '100%', height: 5,
    borderRadius: 2.5, overflow: 'hidden', gap: 2,
  },
